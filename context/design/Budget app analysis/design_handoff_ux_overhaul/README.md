...
> Viewing note: the four `.dc.html` files in this folder ship with `support.js`, `currencies.js` and `deck-stage.js` so the prototype and deck run offline. The audit, deck and mockups also reference the Modernist design-system stylesheet used in the design workspace; if it isn't alongside them, those three still read correctly but lose their document chrome (rules, ground tone). For pixel-accurate review, open them from the design workspace root.
...
## Interactive behavior spec
`Flow Prototype.dc.html` — runnable click-through (sign in → 3-step onboarding → dashboard → Plan / Activity / add-edit forms). Open it in a browser and walk the flow before writing code; the "Decided Money Model" section of `12_UX_OVERHAUL_BUILD_PROMPT.md` states the eight decisions it encodes (accounts model, account-scoped budgets, goals/debts posting to the ledger). It is reference only — never ship the HTML.

## Assets
**Brand — Talvori.** The app is Talvori; the wordmark is set lowercase ("talvori") in Plus Jakarta Sans 800 with -0.02em tracking, and the tagline is "One plan. Everyone. Together."

- **Mark**: `assets/talvori-mark.png` (281×281, transparent). Sidebar 26px beside the wordmark; login 52px in a lockup with the wordmark + tagline; Home/Shop headers 17–34px; app-icon source is the dark rounded tile from the brand sheet (navy #0F172A ground).
- **Palette (use these tokens, not the old indigo set)**: navy `#0F172A` (ink, dark surfaces), purple `#6D4CFF` (primary actions, active nav, hero) with `#5A38E8` pressed and `#EDE7FF` tint, blue `#3B82F6` / `#1D4ED8` on `#EAF2FE` (informational accents), teal `#14B8A6` (income, positive balances, on-track meters; `#0E9384` only where small text needs AA), warm orange `#F59E0B` (shopping and goal accents; `#B45309` for small text), destructive `#DC2626` on `#FEE2E2`. Neutrals: `#475569` secondary text, `#94A3B8` tertiary, `#E2E8F0`/`#EDF1F7` borders, `#F1F5F9`/`#F8FAFF` fills, `#F6F7FB` app background.
- **Type**: Plus Jakarta Sans everywhere (400–800).
- **Rename — already applied in this workspace's copy of the codebase**, mirror it in your repo: `app.json` (`name: "Talvori"`, `slug: "talvori"`), `package.json` + `package-lock.json` (`name: "talvori"`), `locales/en.json` / `fil.json` (`common.appName: "Talvori"`) and `locales/ar.json` (`"تالفوري"`), `app/dev/theme.tsx` SideNav preview brand, plus the doc/comment references in `README.md`, `CLAUDE.md`, `00_README_START_HERE.md`, `09_PHASE_1_CLAUDE_BUILD_PROMPT.md`, `app/(tabs)/_layout.tsx`, `components/theme.ts`, `lib/fonts.ts`, `components/ui/{SideNav,BottomTabBar,Chip,Donut,ProgressRing}.tsx`, `app/finance/budgets.tsx` and the Stitch adoption spec. Zero legacy "ibilly" / "Global Household App" strings remain.
Otherwise no new imagery. Icons: Feather (`@expo/vector-icons`) throughout. Fonts already loaded via `lib/fonts.ts`.
